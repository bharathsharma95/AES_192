...
void aes_enc_dec(unsigned char state[256], unsigned char key[208], unsigned char dir)
{
  unsigned char buf1, buf2, buf3, buf4, buf11, round, i;
  ...
  for (round = 0; round < 12; round++)
    {
    		if (dir)
		{
			...
			key[i] = key[i] ^ key[i-4];
		      ...
		      key[0] = sbox[key[13]]^key[0]^Rcon[11-round];
		      key[1] = sbox[key[14]]^key[1];
		      key[2] = sbox[key[15]]^key[2];
		      key[3] = sbox[key[12]]^key[3]; 
		}
		...
		else
		{
		    ...
		    if ((round > 0 && dir) || (round < 11 && !dir))
			{
    			...
    			} // end " if ((round > 0 && dir) || (round < 11 && !dir)) "
